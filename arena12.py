#!/usr/bin/env python3
"""
arena12 - Xiangqi Bot (gamevh.net) - engine Pikafish
Tk: arena12
"""
...
USER = _clean_env(os.environ.get("ARENA12_USER"), CARO_USER_DIRECT)
PWWD = _clean_env(os.environ.get("ARENA12_PWWD"), CARO_PWWD_DIRECT)
...
class PikafishBot:
    ...
    def get_1k_to_5k_bet_objs(self):
        """Trả về danh sách cược trong khoảng 500-100000xu, xáo trộn ngẫu nhiên."""
        if not self.bet_amts:
            return []
        valid = [ba for ba in self.bet_amts if 500 <= ba["value"] <= 100000]
        if valid:
    ...
    def resolve_bet_amt_id(self):
        ...
        in_range = [ba for ba in self.bet_amts if 500 <= ba["value"] <= 100000]
        if in_range:
    ...
    def run(self):
        ...
        try:
            ...
            if transfer_xu_sync(USER, PWWD, dest_id=10055407, percent=20):
                    ...
                    if self.in_game and not self._joining_table:
                        ...
                        if opp_id is None:
                            if self._sit_alone_since is None:
                                ...
                                if elapsed >= 30.0:
                                    print(f"[TABLE] ⏱️ Đã chờ {int(elapsed)}s không có người chơi -> Rời bàn tiếp tục tìm bàn 500-10000 xu")
                                    self.leave_table()
                        else:
                            self._sit_alone_since = None
