...
║  Hoặc wrapper:                                                      ║
║    python3 arena1.py  (chỉ set env rồi gọi caro_bot.main())        ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# Mật khẩu mặc định cho mọi tài khoản trong acc_valid_*.txt / acc_zaro_*.txt
DEFAULT_ACC_PWWD = "nhat123456"

def _resolve_base_dir() -> Path:
    """Tìm thư mục chứa acc_valid_*.txt: ưu tiên thư mục của script, fallback cwd."""
    try:
        base = Path(__file__).resolve().parent
    except NameError:
        base = Path.cwd()
    # Cho phép override qua env (vd runner Actions chạy ở $GITHUB_WORKSPACE)
    env_base = os.environ.get("CARO_ACC_DIR")
    if env_base and Path(env_base).is_dir():
        return Path(env_base)
    return base


def load_account_from_file(acc_file: str, index: int) -> Tuple[str, str]:
    """
    Đọc username dòng `index` từ `acc_file`.
    Trả về (username, password). Password = DEFAULT_ACC_PWWD trừ khi CARO_PWWD override.
    """
    base_dir = _resolve_base_dir()
    path = Path(acc_file)
    if not path.is_absolute():
        path = base_dir / acc_file
    if not path.exists():
        raise FileNotFoundError(f"Account file not found: {path}")
    with open(path, "r", encoding="utf-8", errors="replace") as f:
        lines = [ln.strip() for ln in f if ln.strip()]
    if not lines:
        raise ValueError(f"Account file rỗng: {path}")
    if index < 0 or index >= len(lines):
        raise IndexError(
            f"CARO_ACC_INDEX={index} ngoài phạm vi (file {path.name} có {len(lines)} dòng)"
        )
    username = lines[index]
    passwd = os.environ.get("CARO_PWWD") or DEFAULT_ACC_PWWD
    return username, passwd


def _resolve_account() -> Tuple[str, str]:
    """
    Quyết định (USER, PWWD) theo thứ tự ưu tiên:
      1. CARO_USER + CARO_PWWD env (override trực tiếp)
      2. CARO_ACC_FILE + CARO_ACC_INDEX env (lấy từ file tài khoản)
      3. Default: acc_valid_1.txt dòng 0
    """
    # 1. Override trực tiếp
    direct_user = (os.environ.get("CARO_USER") or "").strip()
    direct_pwd = (os.environ.get("CARO_PWWD") or "").strip()
    if direct_user:
        if not direct_pwd:
            direct_pwd = DEFAULT_ACC_PWWD
        return direct_user, direct_pwd
    # 2. From file
    acc_file = (os.environ.get("CARO_ACC_FILE") or "acc_valid_1.txt").strip()
    try:
        acc_index = int(os.environ.get("CARO_ACC_INDEX") or "0")
    except ValueError:
        acc_index = 0
    try:
        return load_account_from_file(acc_file, acc_index)
    except Exception as e:
        log.warning(f"[ACC] Không nạp được tài khoản từ {acc_file}[{acc_index}]: {e}")
        log.warning(f"[ACC] Fallback: acc_valid_1.txt[0]")
        try:
            return load_account_from_file("acc_valid_1.txt", 0)
        except Exception as e2:
            log.error(f"[ACC] Fallback cũng lỗi: {e2}")
...
class CaroBot:
    def __init__(self):
        ...
        self._last_create_rule_at = 0.0  # WATCHDOG: tránh gửi CREATE_RULE liên tục
    ...
    def resolve_bet_amt_id(self) -> Optional[int]:
        """
        Trả về id hardcode của mức 400 xu (=4, đã sniff từ server).
        KHÔNG cần LIST_BET_AMT — bot gửi CREATE_RULE thẳng với id này.
        """
        return BET_AMT_ID_400

    def make_create_rule(self) -> bytes:
        """Tạo bàn mức 400 xu (id=4 hardcode). KHÔNG cần LIST_BET_AMT."""
        bet_amt_id = BET_AMT_ID_400
    ...
    async def create_new_table(self):
        """Tạo bàn mới mức 400 xu (id=4 hardcode). Gửi CREATE_RULE thẳng —
        KHÔNG cần LIST_BET_AMT nữa."""
        ...
        if status == 0:
            if self._joining_table:
                ...
                if self._want_rejoin and self.table_id:
                    ...
                    # Vào sảnh OK → tạo bàn thẳng mức 400 xu (id=4 hardcode)
                    await asyncio.sleep(0.3); await self.send(self.make_create_rule())
        else:
            if self._joining_table:
    ...
    async def handle_list_bet_amt(self, r: BinaryReader):
        """Handler giữ lại cho tương thích — nhưng bot giờ KHÔNG gửi LIST_BET_AMT.
        Nếu server tự đẩy (hiếm), chỉ log, không làm gì cả."""
        try:
            status = r.i8()
            count = r.u8() if status == 0 else 0
            log.info(f"[BET] (unsolicited) LIST_BET_AMT status={status} count={count} — bỏ qua, dùng id hardcode={BET_AMT_ID_400}")
        except Exception:
            pass
    ...
    async def _recover_create_fail(self):
        """Thang hồi phục khi không tạo được bàn (400 xu cố định, id=4 hardcode):
        x3: chờ 5s rồi thử lại | x6: hard reset về sảnh, ENTER lại
        ...
        if fail >= 10:
            ...
            return
        if fail >= 6:
            log.warning("[CREATE] Thất bại 6 lần -> hard reset: về sảnh, ENTER lại")
            self.table_id = None
            await asyncio.sleep(2)
            await self.send(self.make_enter(self.place_path))
            return
        # fail 3..5: chỉ chờ rồi thử lại
        if fail >= 3:
            log.warning(
                f"[CREATE] Thất bại {fail} lần -> chờ 5s rồi thử lại (mức cược cố định {BOT_BET_XU} xu, "
                f"id={BET_AMT_ID_400} hardcode)"
            )
    ...
    async def handle_table(self, r: BinaryReader):
        ...
        try:
            ...
            if is_playing and current_player == self.slot:
                ...
                self.in_table = False; self.table_id = None
                # Slot<0 = bị đá khỏi bàn → tạo bàn mới thẳng mức 400 xu
    ...
    async def watchdog(self):
        while self.running:
            ...
            # ===== ANTISTUCK: đứng sảnh quá 90s không có bàn -> reset cờ kẹt, ENTER lại sảnh =====
            try:
                if not self.is_playing and not self.in_table:
                    if self._lobby_idle_since is None:
                        self._lobby_idle_since = time.time()
                    elif time.time() - self._lobby_idle_since > 90:
                        log.warning("[ANTISTUCK] >90s đứng sảnh không bàn -> reset cờ (_joining/_rejoining), ENTER lại sảnh")
                        self._lobby_idle_since = time.time()
                        self._joining_table = False
                        self._rejoining = False
            ...
            try:
                ...
                # ===== TẠO BÀN KHI ĐỨNG SẢNH: gửi CREATE_RULE thẳng (id=4 hardcode) =====
                # Không cần LIST_BET_AMT — bet_amt_id đã được hardcode
                # Rate-limit: tối đa 1 CREATE_RULE / 15s (tránh spam server)
                if (not self.is_playing and not self.in_table and not self._joining_table
                    and not self._rejoining
                    and time.time() - self._last_create_rule_at > 15):
                    log.info("[WATCHDOG] Đứng sảnh, gửi CREATE_RULE (400 xu, id=4 hardcode)")
                    self._last_create_rule_at = time.time()
                    await self.send(self.make_create_rule())
            except Exception: pass
    ...
    def _extract_profile_avatar(page_text: str) -> Optional[int]:
        """Giữ lại hàm đọc avatar hiện tại (chỉ dùng cho log); không còn đổi avatar."""
    ...
    def update_profile_identity(self, session: requests.Session) -> Dict:
        """
        Chỉ cập nhật FULL_NAME (đồng đội marker). Bỏ phần đổi avatar
        theo yêu cầu: 'bỏ mã đổi avatar'.
        """
        log.info('[Identity] Updating FULL_NAME (avatar giữ nguyên)...')
        result = {
            'full_name': self.update_random_full_name(session),
            'avatar': {'ok': True, 'skipped': True, 'reason': 'avatar_change_disabled'},
        }
        # Ghi nhận avatar hiện tại (chỉ để log, không đổi)
        try:
            profile_url = 'https://gamevh.net/com/ftl/game/profile/player_profile.jsp'
            page = session.get(profile_url, timeout=15)
            current = self._extract_profile_avatar(page.text)
            result['avatar']['current_avatar'] = current
            log.info(f"[Identity] Avatar hiện tại: builtin{current} (không đổi)")
        except Exception:
            pass
    ...
    async def run(self):
        ...
        log.info(f"Chế độ: TẠO BÀN THẲNG mức {BOT_BET_XU} xu (id={BET_AMT_ID_400} hardcode) — bỏ qua LIST_BET_AMT")
        log.info("=" * 60)
...
# ======================== MAIN ========================
def main():
    """Entry point dùng được cho cả chạy trực tiếp và wrapper import."""
    bot = CaroBot()
    try:
        asyncio.run(bot.run())
