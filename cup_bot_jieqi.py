...
class JieqiEngine:
    ...
    def get_best_move(self, fen, moves, movetime_ms=5000):
        """Send `position startpos moves ...` then `go movetime {ms}`.
        
        ⚠️ TẠI SAO DÙNG `position startpos` KHÔNG PHẢI `position fen`:
        
        PikaJieQi có 3 bug nghiêm trọng khi dùng `position fen` (đã verify qua source
        code position.cpp + test thực tế):
        
        1. Mất ký tự BAG đầu tiên (line 187 `ss >> token;` đọc 1 char rồi discard)
           - Input BAG: `A2B2N2R2C2P5a2b2n2r2c2p5`
           - Output BAG: `R2C2P5N2B2r2a2c2p5n2b2` (mất A2 = 2 Advisors!)
           - Cup_bot sẽ tính sai số lượng quân còn trong túi
        
        2. Reveal suffix (R/N/C/...) không được apply vào board/BAG
           - Input: `position fen ... moves c3c4R`
           - Output FEN: board vẫn có `X1X1X1X1X` (X chưa revealed thành R)
           - Output BAG: vẫn `R2` (không giảm thành R1)
           - PikaJieQi's `position startpos moves c3c4R` thì apply đúng:
             board có `2R6`, BAG giảm R2→R1
        
        3. Side character bị ignore — luôn ép side=b (BLACK to move)
           - Input: `position fen ... w - - 0 1` (RED to move)
           - Output FEN: `b ...` (BLACK to move — SAI!)
           - Khi bot là RED, engine trả BLACK move → server reject
        
        WORKAROUND (đã verify Task ID 9: 2W/0L, depth 19-25, 0 rejects):
        - Dùng `position startpos` — PikaJieQi's StartFEN có BAG đúng (R2A2C2P5N2B2...)
        - Engine tự generate BAG + track reveals qua moves list
        - Side được track đúng: sau RED move → BLACK to move, etc.
        
        FEN param vẫn được truyền (để giữ API compatible), nhưng KHÔNG được dùng
        trong `position fen` command — chỉ dùng moves list.
        """
        if not self.alive():
            if not self.restart():
                return None
        ...
        # ★ Use "position startpos moves ..." — PikaJieQi's StartFEN matches
        # mystery_xiangqi initial position. Moves list with reveal suffix
        # (e.g. "c3c4R") correctly updates board + BAG + side-to-move.
        try:
            ...
            with self.engine_lock:
                ...
                self.proc.stdin.flush()
                # ★ `go movetime {ms}` works in PikaJieQi (test depth 20 trong 3s)
        ...
        # Wait for bestmove (engine tự stop sau movetime_ms)
        t0 = time.time()
        timeout = (movetime_ms / 1000.0) + 5.0  # movetime + 5s grace
        while time.time() - t0 < timeout:
        ...
        # Timeout — send stop as fallback
        print(f"[ENGINE] bestmove timeout, sending stop")
        try:
            with self.engine_lock:
            ...
            pass
        # Wait up to 2s more for bestmove
        _t = time.time()
        while time.time() - _t < 2.0:
            if self._latest_bestmove:
                break
            time.sleep(0.02)
        self._engine_searching = False
